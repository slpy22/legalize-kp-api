...
- **search_articles**: 키워드 기반 **정확 매칭** 검색. **특정 단어·표현·용어가 법령에 나오는지 찾는 질문에는 반드시 이 도구를 가장 먼저 사용하세요.** (예: "성령을 검색해줘", "OO이라는 표현이 헌법에 있어?", "OO 조항 찾아줘"). 시맨틱/deep_search 는 의미 기반이라 글자가 비슷한 다른 단어에 끌려 정확한 단어를 놓칠 수 있습니다.
...
- **특정 단어·표현·용어 검색** ("성령 검색", "OO이 헌법에 나와?", "OO이라는 말이 있는 조문"): → **search_articles 우선**. 정확 매칭이므로 글자 그대로 찾습니다. 특정 법령 안에서만 찾을 때는 search_articles 의 law_name 인자를 함께 지정하세요 (예: search_articles(query="성령", law_name="조선민주주의인민공화국 헌법")). 이 유형에 deep_search/시맨틱을 쓰지 마세요.
- **분석적/조사 질문** ("기본권 침해", "처벌 조항", "절차", "방법", "관련 법" 등): → **deep_search** 사용.
